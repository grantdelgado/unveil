...
import { testBasicSMS, testGuestInvitationSMS, testUnlinkedGuestScenario } from './test-sms-manual';
...
class SMSDebugger {
  ...
  private updateStep(stepNumber: number, status: DebugStep['status'], result?: any, error?: string, suggestions?: string[]) {
  ...
  private printStep(step: DebugStep) {
    ...
    if (step.suggestions && step.suggestions.length > 0) {
      ...
      step.suggestions.forEach(suggestion => console.log(`   - ${suggestion}`));
    }
  ...
  async runStep1_CheckEnvironmentVariables(): Promise<void> {
    ...
    try {
      ...
      if (missing.length > 0) {
        this.updateStep(1, 'failed', { missing, optionalMissing }, 
      ...
      this.updateStep(1, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep2_ValidateTwilioConfig(): Promise<void> {
    ...
    try {
      ...
      if (validation.isValid) {
        ...
        this.updateStep(2, 'failed', config, 
          'Twilio configuration invalid',
          validation.issues.concat(validation.suggestions)
        );
      }
    } catch (error) {
      this.updateStep(2, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep3_TestTwilioConnection(): Promise<void> {
    ...
    try {
      ...
      if (connected) {
        ...
        this.updateStep(3, 'failed', { connected: false }, 
      ...
      this.updateStep(3, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep4_TestBasicSMS(): Promise<void> {
    ...
    try {
      ...
      this.updateStep(4, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep5_TestGuestInvitationSMS(): Promise<void> {
    ...
    try {
      ...
      this.updateStep(5, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep6_TestUnlinkedGuestScenario(): Promise<void> {
    ...
    try {
      ...
      this.updateStep(6, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep7_CheckDatabaseGuestRecords(): Promise<void> {
    ...
    try {
      ...
      this.updateStep(7, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  async runStep8_VerifySMSOptoutSettings(): Promise<void> {
    ...
    try {
      ...
      this.updateStep(8, 'failed', null, error instanceof Error ? error.message : 'Unknown error');
    }
  }
  ...
  generateReport(): void {
    ...
    if (failed === 0 && passed > 4) {
      ...
      console.log('⚠️ PARTIAL: Some issues found, but core functionality may work');
      console.log('\nPriority fixes needed:');
      this.steps.filter(s => s.status === 'failed').forEach(step => {
...
async function main() {
  ...
  const debugger = new SMSDebugger(testPhone);
