...
async function testSMSAPIEndpoint(): Promise<TestResult[]> {
  ...
  const testGuests = [
    { phone: '+15551234567', guestName: 'Test Guest' }
  ];
  ...
  try {
    ...
    if (response.status === 400 || response.status === 401) {
      results.push({
        ...
        success: true,
        status: response.status,
        details: response.status === 400 ? 'Correctly validates request format' : 'Auth required first'
      });
      console.log(`✅ Correctly handled malformed request (${response.status})`);
    } else {
      results.push({
        test: 'Malformed Request - Should Return 400 or 401',
        success: false,
...
function generateReport(results: TestResult[]) {
  ...
  if (passed === results.length) {
    console.log('🎉 All basic tests passed! The API endpoint structure looks correct.');
    console.log('\n💡 Next steps:');
    ...
    console.log('   3. Verify the authentication token is being passed correctly');
  } else {
    console.log('🔧 Some basic tests failed. Check the API route implementation.');
  }

  console.log('\n📚 Resources:');
  console.log('   API Route: app/api/sms/send-invitations/route.ts');
  console.log('   Client Function: lib/api/sms-invitations.ts');
  console.log('   Debugging Guide: docs/debugging/sms-invitation-troubleshooting.md');
}
