...
function testScrollBehavior() {
  ...
  if (isScrollable) {
    console.log(
      '⬆️  Content exceeds viewport - scrolling should work normally',
    );
  } else {
    ...
    setTimeout(() => {
      ...
      if (newScrollTop === initialScrollTop) {
        ...
        console.log(
          "❌ Page scrolled when it shouldn't - check implementation",
        );
      }
  ...
  if (!isScrollable && !hasVerticalScrollbar) {
    ...
    console.log(
      'ℹ️  INFO: Content exceeds viewport - scrolling enabled as expected',
    );
  } else {
