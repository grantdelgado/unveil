...
async function main() {
  ...
  try {
    ...
    console.log('📊 Scanning for scheduled messages with incorrect recipient counts...');
    
    const { data: scheduledMessages, error: fetchError } = await supabase
      .from('scheduled_messages')
      .select('id, event_id, status, target_guest_ids, recipient_count, created_at, send_at')
    ...
    console.log(`📋 Found ${scheduledMessages.length} scheduled messages to examine.`);
    ...
    console.log(`🔧 Found ${incorrectMessages.length} messages with incorrect recipient counts:`);
    console.log('');
    ...
    incorrectMessages.forEach((msg, index) => {
      ...
      console.log(`   Will update recipient_count: ${storedCount} → ${actualCount}`);
      console.log('');
    });

    if (isDryRun) {
      console.log('🔍 DRY RUN COMPLETE - No changes made.');
      console.log(`Would update ${incorrectMessages.length} scheduled messages.`);
      return;
    }

    // Step 4: Confirm before proceeding
    console.log('⚠️  About to update recipient counts for the messages listed above.');
    ...
    for (const msg of incorrectMessages) {
      ...
      try {
        console.log(`   Updating message ${msg.id}: recipient_count ${msg.recipient_count} → ${actualCount}`);
    ...
    const { data: remainingIncorrect } = await supabase
      ...
      .in('status', ['scheduled', 'sending'])
      .not('target_guest_ids', 'is', null) as { data: ScheduledMessageRow[] | null };

    const stillIncorrect = remainingIncorrect?.filter(msg => {
      const actualCount = msg.target_guest_ids?.length || 0;
      const storedCount = msg.recipient_count || 0;
      return actualCount !== storedCount && actualCount > 0;
    }) || [];
    ...
    if (stillIncorrect.length === 0) {
      console.log('✅ Verification passed: All scheduled messages now have correct recipient counts.');
    } else {
      console.log(`⚠️  Verification found ${stillIncorrect.length} messages still with incorrect counts.`);
      console.log('These may have been updated concurrently or had other issues.');
    }
