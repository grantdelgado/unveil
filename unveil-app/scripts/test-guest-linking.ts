...
async function testGuestLinkingAPI(): Promise<TestResult[]> {
  ...
  try {
    ...
    const response = await fetch(
      'http://localhost:3000/api/guests/link-unlinked',
      {
        method: 'POST',
        headers: {
  ...
  try {
    ...
    const { linkGuestRecordsToUser } = await import(
      '../lib/db/linkGuestRecords'
    );

    // Test with invalid phone number
    const invalidResult = await linkGuestRecordsToUser(
      'test-user-id',
      'invalid-phone',
      false,
    );
    ...
    const validResult = await linkGuestRecordsToUser(
      'test-user-id',
      '+15551234567',
      false,
    );
    ...
    console.log(
      `❌ Function test error: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
...
function generateReport(results: TestResult[]) {
  ...
  if (passed === results.length) {
    console.log(
      '🎉 All tests passed! Guest linking API should be working correctly.',
    );
    console.log('\n💡 Next steps:');
    ...
    console.log(
      '🔧 Some tests failed. Check the errors above and fix before testing with real users.',
    );
  }
