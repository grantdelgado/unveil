  ...
  for (const [deviceName, viewport] of Object.entries(VIEWPORT_CONFIGS)) {
    ...
    for (const url of TEST_URLS) {
      ...
      try {
        ...
        testResult.loadTime = Math.round(
          performanceMetrics.DOMContentLoaded * 1000,
        );

        // Test touch targets (minimum 44px)
        const touchTargetResults = await page.evaluate(() => {
          const interactiveElements = document.querySelectorAll(
            'button, input, [role="button"], [role="tab"], a, select, textarea',
          );
          const results = [];
            ...
            if (isVisible) {
              results.push({
                ...
                text:
                  ...
                  element.getAttribute('aria-label')?.slice(0, 30) ||
                  'No text',
              });
            }
          });
        ...
        testResult.responsiveLayout = await page.evaluate(() => {
          ...
          const hasOverflowContent = document.querySelector(
            '[style*="overflow-x"]',
          );
  ...
  for (const [device, deviceResults] of Object.entries(results)) {
    ...
    for (const [url, result] of Object.entries(deviceResults)) {
      if (result.error) {
        ...
        const touchTargetIssues = result.touchTargets.filter(
          (t) => !t.meetsMinimum,
        );
        const loadTimeOk = result.loadTime < 3000;

        console.log(
          `  ${loadTimeOk && result.responsiveLayout && result.criticalContentVisible && touchTargetIssues.length === 0 ? '✅' : '⚠️'} ${url}:`,
        );
        console.log(
          `    Load Time: ${result.loadTime}ms ${loadTimeOk ? '✅' : '⚠️'}`,
        );
        console.log(`    Responsive: ${result.responsiveLayout ? '✅' : '❌'}`);
        console.log(
          `    Critical Content: ${result.criticalContentVisible ? '✅' : '❌'}`,
        );
        console.log(
          `    Touch Targets: ${result.touchTargets.length - touchTargetIssues.length}/${result.touchTargets.length} pass ${touchTargetIssues.length === 0 ? '✅' : '⚠️'}`,
        );

        if (touchTargetIssues.length > 0) {
          console.log(
            `    Small targets: ${touchTargetIssues
              .slice(0, 3)
              .map((t) => `${t.width}x${t.height}px`)
              .join(', ')}`,
          );
          allTestsPass = false;
        }
