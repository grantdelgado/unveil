...
function addResult(result: SecurityTest) {
  ...
  console.log(
    `${emoji} [${result.category}] ${result.name}: ${result.message}${risk}`,
  );
  results.push(result);
}
...
async function testFileUploadSecurity() {
  ...
  try {
    const maliciousFile = new Blob(['<?php echo "hack"; ?>'], {
  ...
  try {
    ...
    const largeFile = new Blob([new ArrayBuffer(52 * 1024 * 1024)], {
      type: 'image/jpeg',
    });
    const { error } = await supabase.storage
      .from('event-media')
    ...
    if (
      error &&
      (error.message.includes('size') || error.message.includes('limit'))
    ) {
      addResult({
        category: 'FILE_UPLOAD',
