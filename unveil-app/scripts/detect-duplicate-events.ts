...
function displayDuplicates(duplicates: DuplicateGroup[]) {
  ...
  duplicates.forEach((group, index) => {
    ...
    if (group.timing_analysis) {
      ...
      console.log(
        `   Created: ${timing_analysis.first_created} → ${timing_analysis.last_created}`,
      );
      console.log(`   Time Gap: ${timing_analysis.time_difference_ms}ms`);
      console.log(
        `   Likely Double-Submit: ${timing_analysis.likely_double_submission ? '🔴 YES' : '🟡 NO'}`,
      );
    }
...
async function analyzeCreationPatterns(duplicates: DuplicateGroup[]) {
  ...
  const doubleSubmissions = duplicates.filter(
    (d) => d.timing_analysis?.likely_double_submission,
  );
  const slowDuplicates = duplicates.filter(
    (d) => !d.timing_analysis?.likely_double_submission,
  );
  ...
  console.log(
    `• Total affected hosts: ${new Set(duplicates.map((d) => d.host_user_id)).size}`,
  );
  ...
  if (doubleSubmissions.length > 0) {
    ...
    doubleSubmissions.forEach((d) => {
      console.log(
        `   • ${d.host_name}: ${d.timing_analysis!.time_difference_ms}ms gap`,
      );
    });
  }
...
async function suggestCleanup(duplicates: DuplicateGroup[]) {
  ...
  const safeToCleaup = duplicates.filter((d) => {
    return (
      d.timing_analysis?.likely_double_submission && d.duplicate_count === 2
    );
  });
  ...
  console.log(
    '   2. Check for any media or messages associated with duplicates',
  );
  console.log(
    '   3. Run: npx tsx scripts/detect-duplicate-events.ts --fix-duplicates',
  );
}
...
async function performCleanup(duplicates: DuplicateGroup[]) {
  ...
  for (const group of safeToCleaup) {
    ...
    for (const deleteId of deleteIds) {
      ...
      if (guestsError) {
        console.log(
          `  ❌ Error checking guests for ${deleteId}: ${guestsError.message}`,
        );
        continue;
      }
      ...
      if (nonHostGuests.length > 0) {
        console.log(
          `  ⚠️  Skipping ${deleteId}: has ${nonHostGuests.length} non-host guests`,
        );
        continue;
      }
