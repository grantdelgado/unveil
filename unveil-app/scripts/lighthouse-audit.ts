...
function printReport(report: LighthouseReport) {
  ...
  const scoreEmoji = (score: number, threshold: number) =>
    score >= threshold ? '✅' : '❌';

  console.log(
    `│ Performance:    ${report.scores.performance.toString().padStart(3)} ${scoreEmoji(report.scores.performance, PERFORMANCE_THRESHOLDS.performance)} (≥${PERFORMANCE_THRESHOLDS.performance})       │`,
  );
  console.log(
    `│ Accessibility:  ${report.scores.accessibility.toString().padStart(3)} ${scoreEmoji(report.scores.accessibility, PERFORMANCE_THRESHOLDS.accessibility)} (≥${PERFORMANCE_THRESHOLDS.accessibility})       │`,
  );
  console.log(
    `│ Best Practices: ${report.scores.bestPractices.toString().padStart(3)} ${scoreEmoji(report.scores.bestPractices, PERFORMANCE_THRESHOLDS.bestPractices)} (≥${PERFORMANCE_THRESHOLDS.bestPractices})       │`,
  );
  console.log(
    `│ SEO:           ${report.scores.seo.toString().padStart(3)} ${scoreEmoji(report.scores.seo, PERFORMANCE_THRESHOLDS.seo)} (≥${PERFORMANCE_THRESHOLDS.seo})       │`,
  );
  console.log('├─────────────────────────────────────────┤');
  ...
  console.log(
    `│ FCP:  ${formatMetric(report.metrics.firstContentfulPaint, 'ms').padStart(10)} ${metricEmoji(report.metrics.firstContentfulPaint, PERFORMANCE_THRESHOLDS.firstContentfulPaint, true)}        │`,
  );
  console.log(
    `│ LCP:  ${formatMetric(report.metrics.largestContentfulPaint, 'ms').padStart(10)} ${metricEmoji(report.metrics.largestContentfulPaint, PERFORMANCE_THRESHOLDS.largestContentfulPaint, true)}        │`,
  );
  console.log(
    `│ CLS:  ${formatMetric(report.metrics.cumulativeLayoutShift, '').padStart(10)} ${metricEmoji(report.metrics.cumulativeLayoutShift, PERFORMANCE_THRESHOLDS.cumulativeLayoutShift, true)}        │`,
  );
  console.log(
    `│ SI:   ${formatMetric(report.metrics.speedIndex, 'ms').padStart(10)} ${metricEmoji(report.metrics.speedIndex, PERFORMANCE_THRESHOLDS.speedIndex, true)}        │`,
  );
  console.log(
    `│ TBT:  ${formatMetric(report.metrics.totalBlockingTime, 'ms').padStart(10)} ${metricEmoji(report.metrics.totalBlockingTime, PERFORMANCE_THRESHOLDS.totalBlockingTime, true)}        │`,
  );
  console.log('└─────────────────────────────────────────┘');
}
  ...
  if (failedReports.length > 0) {
    ...
    failedReports.forEach((report) => {
      ...
      if (report.scores.performance < PERFORMANCE_THRESHOLDS.performance) {
        console.log(
          `     - Performance: ${report.scores.performance} (below ${PERFORMANCE_THRESHOLDS.performance})`,
        );
      }
      if (report.scores.accessibility < PERFORMANCE_THRESHOLDS.accessibility) {
        console.log(
          `     - Accessibility: ${report.scores.accessibility} (below ${PERFORMANCE_THRESHOLDS.accessibility})`,
        );
      }
    });
  }
