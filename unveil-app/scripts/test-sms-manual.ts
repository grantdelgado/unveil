...
async function testBasicSMS(phone: string): Promise<TestResult> {
  ...
  try {
    const result = await sendSMS({
      to: phone,
      message: 'Test SMS from Unveil debug script. This confirms your SMS setup is working!',
      eventId: TEST_EVENT_ID,
      messageType: 'custom'
    });

    if (result.success) {
      console.log(`✅ Basic SMS sent successfully! Message ID: ${result.messageId}`);
      return {
        testName: 'Basic SMS',
        success: true,
...
async function testUnlinkedGuestScenario(phone: string): Promise<TestResult> {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log('   This confirms unlinked guests (user_id = NULL) can receive SMS');
      return {
...
function generateTestReport(results: TestResult[]) {
  ...
  if (passed === results.length) {
    ...
    console.log('   4. Ensure the guest import process is including all guests');
  } else {
    console.log('\n🔧 Next Steps:');
    ...
    console.log('   4. Run the config debug script: npx tsx scripts/debug-sms-config.ts');
  }
}
...
async function main() {
  ...
  console.log('   Twilio Message Logs: https://console.twilio.com/us1/develop/sms/logs');
