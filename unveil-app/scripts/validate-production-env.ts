    ...
    result.status === 'PASS'
      ...
      : result.status === 'FAIL'
        ...
        : result.status === 'WARN'
          ...
          : 'ℹ️';
  console.log(
    `${emoji} [${result.category}] ${result.name}: ${result.message}`,
  );
  results.push(result);
}
    ...
    if (error) {
      if (error.message.includes('not found')) {
        addResult({
          ...
          status: 'FAIL',
          message:
            'event-media bucket not found - create manually in Supabase Dashboard',
          critical: true,
        });
      } else {
        addResult({
...
function validateOptionalServices() {
  const services = [
    ...
    },
    {
      ...
      env: process.env.NEXT_PUBLIC_GA_TRACKING_ID,
      category: 'ANALYTICS',
    },
    {
      name: 'LogRocket',
      env: process.env.NEXT_PUBLIC_LOGROCKET_ID,
      category: 'MONITORING',
    },
  ];
