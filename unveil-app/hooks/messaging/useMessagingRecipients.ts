...
export function useMessagingRecipients(
  ...
  const fetchRecipients = useCallback(async () => {
    ...
    try {
      ...
      if (rpcError) {
        throw new Error(
          `Failed to fetch messaging recipients: ${rpcError.message}`,
        );
      }

      const processedRecipients = (Array.isArray(data) ? data : []).map(
        (recipient: any) => ({
      ...
      logger.error('Error fetching messaging recipients', {
        eventId,
        error: errorMessage,
      });
      ...
      if (errorMessage.includes('Could not find the function')) {
        userFriendlyError =
          'Messaging service is temporarily unavailable. Please refresh the page.';
      } else if (errorMessage.includes('Access denied')) {
        userFriendlyError =
          'You don&apos;t have permission to view recipients for this event.';
      } else if (
