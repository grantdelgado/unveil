...
export function useScheduledMessagesCache({
  ...
  const addMessage = useCallback(
    (message: ScheduledMessage) => {
      ...
      if (processedMessageIds.current.has(message.id)) {
        console.log(
          `🔄 Skipping duplicate manual add for message ${message.id}`,
        );
        return;
      }
      ...
      queryClient.setQueryData(
        queryKey,
        (old: ScheduledMessage[] | undefined) => {
  ...
  const handleRealtimeInsert = useCallback(
    (newMessage: ScheduledMessage): boolean => {
      // Check for duplicates
      if (processedMessageIds.current.has(newMessage.id)) {
        console.log(
          `🔄 Skipping duplicate INSERT for message ${newMessage.id}`,
        );
        return false;
      }
  ...
  const handleRealtimeDelete = useCallback(
    (deletedMessage: ScheduledMessage) => {
      // Optimistically update React Query cache
      queryClient.setQueryData(
        queryKey,
        (old: ScheduledMessage[] | undefined) => {
