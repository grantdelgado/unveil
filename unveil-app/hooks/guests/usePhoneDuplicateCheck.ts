...
export function usePhoneDuplicateCheck(eventId: string) {
  ...
  const checkPhoneDuplicate = useCallback(async (phone: string): Promise<PhoneDuplicateCheckResult> => {
    if (!phone.trim()) {
    ...
    try {
      const { data: isDuplicate, error } = await supabase
        .rpc('check_phone_exists_for_event', {
          p_event_id: eventId,
  ...
  const isChecking = useCallback((phone: string): boolean => {
    const normalizedPhone = normalizePhoneNumber(phone);
    const cacheKey = `${eventId}:${normalizedPhone}`;
    return checkingPhones.has(cacheKey);
  }, [eventId, checkingPhones]);

  const getError = useCallback((phone: string): string | null => {
    const normalizedPhone = normalizePhoneNumber(phone);
    const cacheKey = `${eventId}:${normalizedPhone}`;
