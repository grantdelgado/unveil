...
test.describe('MVP Critical User Flows', () => {
  test.beforeAll(async () => {
    // Set up test users
    const host = await addTestUser(
      'e2e-host@test.com',
      '+1234567800',
      'Host User',
    );
    const guest = await addTestUser(
      'e2e-guest@test.com',
      '+1234567801',
      'Guest User',
    );
  ...
  test('Complete Host Workflow: Create Event → Import Guests → Send Invites', async ({
    page,
  }) => {
    ...
    await test.step('Create New Event', async () => {
      ...
      await expect(
        page.locator('[data-testid="event-created-success"]'),
      ).toBeVisible();
    ...
    await test.step('Import Guest List', async () => {
      ...
      // Verify guest list
      await expect(page.locator('[data-testid="guest-list-item"]')).toHaveCount(
        TEST_GUESTS.length,
      );
    });
    ...
    await test.step('Send SMS Invitations', async () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        await expect(
          page.locator('[data-testid="sms-test-panel"]'),
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="mock-sms-count"]'),
        ).toContainText('3');
      }

      // Verify invitation status
      await expect(
        page.locator('[data-testid="invitation-sent-status"]'),
      ).toHaveCount(TEST_GUESTS.length);
    });
  });

  test('Complete Guest Workflow: Receive Invite → RSVP → Upload Media → Send Messages', async ({
    page,
  }) => {
    ...
    await test.step('Access Event via Invitation Link', async () => {
      ...
      await expect(page.locator('h1')).toContainText(TEST_EVENT.title);
      await expect(page.locator('[data-testid="event-date"]')).toContainText(
        'December 25, 2024',
      );
    });
    ...
    await test.step('RSVP to Event', async () => {
      ...
      await expect(page.locator('[data-testid="rsvp-status"]')).toContainText(
        'Attending',
      );
    });
    ...
    await test.step('Upload Event Media', async () => {
      ...
      await expect(
        page.locator('[data-testid="media-gallery-item"]'),
      ).toHaveCount(1);
    });
  ...
  test('Cross-Device Real-time Synchronization', async ({ browser }) => {
    ...
    await test.step('Test Real-time Message Sync', async () => {
      ...
      await expect(
        guestPage.locator('[data-testid="message-item"]').last(),
      ...
      await expect(
        hostPage.locator('[data-testid="message-item"]').last(),
    ...
    await test.step('Test RSVP Sync', async () => {
      ...
      await expect(
        hostPage.locator('[data-testid="rsvp-count-attending"]'),
  ...
  test('Mobile-Specific Features', async ({ page }) => {
    ...
    await test.step('Mobile Media Upload (Camera)', async () => {
      ...
      await expect(
        page.locator('[data-testid="camera-permission-prompt"]'),
      ).toBeVisible();
    });
  ...
  test('Error Handling and Recovery', async ({ page }) => {
    ...
    await test.step('File Upload Error Handling', async () => {
      ...
      await page
        .locator('[data-testid="media-upload-input"]')
        .setInputFiles(oversizedFile);
  ...
  test('Performance Validation', async ({ page }) => {
    ...
    await test.step('Media Upload Performance', async () => {
      ...
      await page
        .locator('[data-testid="media-upload-input"]')
        .setInputFiles('src/test/fixtures/test-image.jpg');
      await expect(page.locator('[data-testid="upload-success"]')).toBeVisible({
        timeout: 10000,
      });
