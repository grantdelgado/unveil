...
test.describe('Select Event Page Scroll Behavior', () => {
  ...
  test('should not scroll when content fits within viewport - small dataset', async ({
    page,
  }) => {
    ...
    await page.waitForSelector(
      '[data-testid="event-card"], h1:has-text("Welcome!")',
      { timeout: 10000 },
    );
  ...
  test('should scroll naturally when content exceeds viewport - large dataset', async ({
    page,
  }) => {
  ...
  test('should handle safe areas properly on mobile viewports', async ({
    page,
  }) => {
    // Set mobile viewport
    ...
    const usesCorrectViewportHeight = await page.evaluate(() => {
      const elements = document.querySelectorAll(
        '[class*="min-h-mobile"], [class*="h-mobile"]',
      );
