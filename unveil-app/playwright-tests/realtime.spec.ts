...
test.describe('Real-Time Features E2E Tests', () => {
  ...
  test('should broadcast messages in real-time between host and guest', async () => {
    ...
    await hostPage.waitForSelector('[data-testid="message-composer"]', {
      timeout: 10000,
    });
    ...
    const hostMessageElement = await hostPage.textContent(
      `text=${hostMessage}`,
    );
    const guestMessageElement = await guestPage.textContent(
      `text=${hostMessage}`,
    );
  ...
  test('should show real-time message updates', async () => {
    ...
    const messageElement = await hostPage
      .locator(`text=${initialMessage}`)
      .first();

    if (await messageElement.isVisible()) {
      // Try to find edit button
      const editButton = await messageElement
        .locator('..')
        .locator('[data-testid="edit-message"]');
      ...
      if (await editButton.isVisible()) {
        ...
        await guestPage.waitForSelector(`text=${updatedMessage}`, {
          timeout: 10000,
        });

        const updatedElement = await guestPage.textContent(
          `text=${updatedMessage}`,
        );
  ...
  test('should broadcast media uploads in real-time', async () => {
    ...
    const mediaUploadSection = await hostPage.locator(
    ...
    if (await mediaUploadSection.isVisible()) {
      ...
      if (await fileInput.isVisible()) {
        ...
        // Submit upload
        const uploadButton = await hostPage.locator(
        ...
        if (await uploadButton.isVisible()) {
          ...
          await hostPage.waitForSelector('[data-testid="media-item"]', {
            timeout: 10000,
          });

          // Check if media appears on guest page in real-time
          await guestPage.waitForSelector('[data-testid="media-item"]', {
            timeout: 15000,
          });

          // Verify media is visible on both pages
          const hostMediaCount = await hostPage
            .locator('[data-testid="media-item"]')
            .count();
          const guestMediaCount = await guestPage
            .locator('[data-testid="media-item"]')
            .count();
  ...
  test('should handle participant RSVP updates in real-time', async () => {
    ...
    if (await rsvpSection.isVisible()) {
      ...
      const attendingButton = await guestPage.locator(
      ...
      if (await attendingButton.isVisible()) {
        ...
        await guestPage.waitForSelector('[data-testid="rsvp-confirmed"]', {
          timeout: 5000,
        });

        // Check if host page shows updated participant count/status
        await hostPage.waitForSelector('[data-testid="participant-stats"]', {
          timeout: 10000,
        });

        // Look for indication of RSVP update (this depends on UI implementation)
        const participantStats = await hostPage.textContent(
  ...
  test('should maintain real-time connection after network reconnection', async () => {
    ...
    const reconnectedMessage = await guestPage.textContent(
      `text=${afterMessage}`,
    );
  ...
  test('should display real-time notifications for new activity', async () => {
    ...
    const notificationCenter = await hostPage.locator(
    ...
    if (await notificationCenter.isVisible()) {
      ...
      await hostPage.waitForSelector('[data-testid="new-notification"]', {
        timeout: 10000,
      });

      // Verify notification content
      const notification = await hostPage.textContent(
        '[data-testid="new-notification"]',
      );
      expect(notification).toBeTruthy();

      // Check notification counter
      const notificationCount = await hostPage.locator(
