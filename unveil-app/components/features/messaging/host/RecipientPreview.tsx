...
import type { RecipientPreviewData, FilteredGuest } from '@/lib/types/messaging';
...
export function RecipientPreview({
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-3 flex-1 min-w-0">
          ...
          {!guest.hasPhone && (
            <span className="text-xs text-red-500 flex-shrink-0" title="No phone number - will be skipped">
              📵
            </span>
          )}
        </div>
        ...
        {guest.tags.length > 0 && (
          <div className="flex items-center space-x-1 ml-2 flex-shrink-0">
            ...
            {guest.tags.length > 2 && (
              <span className="text-xs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-full" title={`+${guest.tags.length - 2} more: ${guest.tags.slice(2).join(', ')}`}>
                +{guest.tags.length - 2}
              </span>
            )}
          </div>
  ...
  const SummaryStats = ({ data }: { data: RecipientPreviewData }) => {
    ...
    const percentageValid = data.totalCount > 0 ? Math.round((data.validRecipientsCount / data.totalCount) * 100) : 0;
      ...
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-3 mb-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div className="text-center sm:text-left">
            ...
            <div className="text-lg font-bold text-purple-700">{data.totalCount}</div>
          </div>
          <div className="text-center sm:text-left">
            <div className="font-medium text-purple-900">Will Receive Message</div>
            <div className="text-lg font-bold text-green-700">{data.validRecipientsCount}</div>
          </div>
          <div className="text-center sm:text-left">
            <div className="font-medium text-purple-900">Success Rate</div>
            <div className="text-lg font-bold text-blue-700">{percentageValid}%</div>
          </div>
        </div>
        
        {invalidPhoneCount > 0 && (
          <div className="mt-3 text-xs text-amber-800 bg-amber-100 border border-amber-200 rounded px-3 py-2">
            <span className="font-medium">⚠️ Notice:</span> {invalidPhoneCount} guest{invalidPhoneCount !== 1 ? 's' : ''} will be skipped (missing phone number)
          </div>
        )}
        
        {data.totalCount > 100 && (
          <div className="mt-2 text-xs text-blue-700 bg-blue-100 rounded px-2 py-1">
            ⚡ Large list detected - using optimized rendering for best performance
          </div>
        )}
      </div>
    );
  ...
  if (!previewData) {
    return (
      <div className={cn('border rounded-lg bg-white', className)}>
        <div className="p-4">
          <EmptyState 
            message="Loading recipient preview..." 
            icon="⏳" 
          />
        </div>
      </div>
      ...
      <div className="p-4">
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            >
              ...
              {endIndex < guests.length && (
                <div style={{ height: (guests.length - endIndex) * ITEM_HEIGHT }} />
              )}
            </div>
            ...
            {totalCount > 8 && (
              <div className="bg-gray-50 border-t border-gray-200 px-3 py-2">
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>
                    Showing {Math.min(endIndex - startIndex, visibleGuests.length)} of {totalCount} guests
                  </span>
                  {totalCount > 8 && (
        ...
        {validRecipientsCount > 0 && (
          <div className="mt-3 text-xs text-gray-500 bg-gray-50 rounded px-3 py-2">
            💬 Messages will be sent via push notification to {validRecipientsCount} recipient{validRecipientsCount !== 1 ? 's' : ''}
          </div>
        )}
      </div>
    </div>
