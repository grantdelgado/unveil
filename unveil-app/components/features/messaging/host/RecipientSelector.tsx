...
export function RecipientSelector({
  ...
  const [activeTab, setActiveTab] = useState<'rsvp' | 'tags' | 'combined'>(
    'rsvp',
  );
  const {
    tags: availableTags,
  ...
  const handleRsvpChange = (status: string, checked: boolean) => {
    ...
    const newFilter: RecipientFilter = {
      ...filter,
      type:
        newStatuses.length === 0 && !filter.tags?.length ? 'all' : 'combined',
  ...
  const handleTagChange = (tag: string, checked: boolean) => {
    ...
    const newFilter: RecipientFilter = {
      ...filter,
      type:
        newTags.length === 0 && !filter.rsvpStatuses?.length
          ? 'all'
          : 'combined',
  ...
  const getActiveFilterSummary = () => {
    ...
    if (selectedRsvpCount > 0) {
      parts.push(
        `${selectedRsvpCount} RSVP status${selectedRsvpCount > 1 ? 'es' : ''}`,
      );
    }
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <FieldLabel className="text-gray-700 font-medium">Send To</FieldLabel>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <Button
          variant={
            filter.rsvpStatuses?.includes('pending') &&
            filter.rsvpStatuses.length === 1
              ? 'secondary'
              : 'outline'
          }
          size="sm"
        ...
        <Button
          variant={
            filter.rsvpStatuses?.includes('attending') &&
            filter.rsvpStatuses.length === 1
              ? 'secondary'
              : 'outline'
          }
                ...
                {rsvpOptions.map((option) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={
                        filter.rsvpStatuses?.includes(option.value) || false
                      }
                      onChange={(e) =>
                        handleRsvpChange(option.value, e.target.checked)
                      }
              ...
              {selectedRsvpCount > 1 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="text-xs font-medium text-blue-900 mb-2">
                    Multiple RSVP statuses selected - guests matching ANY of
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs opacity-75">
                          Match guests with ANY selected tag
                        </div>
                      </div>
                    </label>
                    <label
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs opacity-75">
                          Match guests with ALL selected tags
                        </div>
                      </div>
                    </label>
                  </div>
              ...
              {hasMultipleFilters && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-2">
                  <div className="text-xs text-amber-800">
                    <span className="font-medium">💡 Combining filters:</span>{' '}
                    Guests matching{' '}
                    {selectedRsvpCount > 0 && selectedTagCount > 0 && (
                      <>
                        <span className="font-medium">
                          {filter.requireAllTags ? 'ALL' : 'ANY'} selected tags
                          AND ANY selected RSVP status
                        </span>
                      </>
                    )}
                  </div>
