...
export class RetryManager {
  ...
  static isRetryableError(
    ...
    if (
      failureReason &&
      this.RETRYABLE_FAILURE_REASONS.includes(failureReason)
    ) {
      return true;
    }
  ...
  static async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= opts.maxAttempts; attempt++) {
      try {
        ...
        if (attempt > 1) {
          logger.debug(
            `✅ Retry successful after ${attempt} attempts (${duration}ms)`,
            {
        ...
        logger.debug(
          `⚠️ Retry attempt ${attempt}/${opts.maxAttempts} failed, retrying in ${delay}ms`,
          {
            context: opts.context,
            error: lastError,
            delay,
    ...
    const duration = Date.now() - startTime;
    logger.warn(
      `❌ All retry attempts failed after ${opts.maxAttempts} attempts (${duration}ms)`,
      {
        context: opts.context,
        error: lastError,
  ...
  static async executeWithContextRetry<T>(
    ...
    for (let attempt = 1; attempt <= opts.maxAttempts; attempt++) {
      try {
        ...
        if (result.success) {
          ...
          if (attempt > 1) {
            logger.debug(
              `✅ Context retry successful after ${attempt} attempts (${duration}ms)`,
              {
                context,
                attempts: attempt,
                duration,
        ...
        logger.debug(
          `⚠️ Context retry attempt ${attempt}/${opts.maxAttempts} failed, retrying in ${delay}ms`,
          {
            context,
            error: result.error,
        ...
        logger.debug(
          `⚠️ Context retry attempt ${attempt}/${opts.maxAttempts} threw error, retrying in ${delay}ms`,
          {
            context,
            error: lastError,
            delay,
    ...
    const duration = Date.now() - startTime;
    logger.warn(
      `❌ All context retry attempts failed after ${opts.maxAttempts} attempts (${duration}ms)`,
      {
        context,
        error: lastError,
