...
export async function autoJoinInvitedGuests(
  ...
  try {
    ...
    if (checkError) {
      logger.apiError(
        'Failed to check existing events',
        checkError,
        'guestAutoJoin.autoJoinInvitedGuests',
      );
    } else if (existingEvents && existingEvents.length > 0) {
      logger.api(
        'User already has linked events, skipping auto-join',
        {
          userId,
    ...
    logger.api(
      'Starting bulk auto-join process',
      {
        userId,
    ...
    if (rpcError) {
      ...
      if (errorCode === '42883') {
        return {
          ...
          joinedEvents: [],
          error:
            'Function not found - bulk_guest_auto_join may not be deployed',
        };
      }
    ...
    if (!result || typeof result !== 'object') {
      logger.apiError(
        'Invalid bulk auto-join RPC response',
        { result },
        'guestAutoJoin.autoJoinInvitedGuests',
      );
      return {
    ...
    if (resultData.status === 'success') {
      const linkedEvents = Array.isArray(resultData.linked_events)
        ? resultData.linked_events
        : [];
      const alreadyLinkedEvents = Array.isArray(
        resultData.already_linked_events,
      )
        ? resultData.already_linked_events
        : [];

      logger.api(
        'Bulk auto-join completed successfully',
        {
          userId,
          totalLinked: resultData.total_linked || 0,
          totalAlreadyLinked: resultData.total_already_linked || 0,
          linkedEvents,
          alreadyLinkedEvents,
        },
        'guestAutoJoin.autoJoinInvitedGuests',
...
export async function autoJoinEventByPhone(
  ...
  try {
    ...
    switch (resultData.status) {
      case 'linked':
        logger.api(
          'Successfully linked guest to event',
          {
            eventId,
            userId,
      ...
      case 'conflict':
        logger.api(
          'Invitation already claimed by different user',
          {
            eventId,
            userId,
          },
    ...
    logger.apiError(
      'Auto-join event process failed',
      error,
      'guestAutoJoin.autoJoinEventByPhone',
    );
    return {
...
export async function getVisibleEventsForUser(
  ...
  try {
    ...
    if (error) {
      logger.apiError(
        'Failed to fetch visible events',
        error,
        'guestAutoJoin.getVisibleEventsForUser',
      );
    ...
    return {
      ...
      events: [],
      error:
        error instanceof Error ? error.message : 'Failed to get visible events',
    };
  }
}
