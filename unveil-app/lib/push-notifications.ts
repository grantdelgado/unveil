...
export async function sendPushNotification(payload: PushNotificationPayload): Promise<PushResult> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      console.log(`📱 Sending push notification to ${redactedToken}:`, payload.title);
      ...
      if (result.success) {
        if (attempt > 0) {
          console.log(`✅ Push notification sent successfully on retry ${attempt + 1} for token ${redactedToken}`);
        }
        return result;
      }
      ...
      console.log(`⚠️ Retryable push error for token ${redactedToken}, attempt ${attempt + 1}/${maxRetries}: ${lastError}`);
      ...
      console.error(`❌ Exception during push send attempt ${attempt + 1} for token ${redactedToken}:`, lastError);
...
async function sendAPNSNotification(payload: PushNotificationPayload): Promise<PushResult> {
  try {
    ...
    console.log('⚠️ APNS implementation is simplified - would send:', {
...
export async function getDeviceTokensForGuests(
  ...
  try {
    ...
    console.log(`📱 Would fetch device tokens for ${guestIds.length} guests in event ${eventId}`);
...
export async function sendBulkScheduledPush(
  ...
  console.log(`📱 Sending scheduled push notifications to ${deliveries.length} recipients`);
  ...
  console.log(`✅ Bulk scheduled push complete: ${successful} delivered, ${failed} failed`);
