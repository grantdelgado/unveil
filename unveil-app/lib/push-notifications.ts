...
export async function sendPushNotification(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      logger.system(
        `Sending push notification to ${redactedToken}`,
        payload.title,
      );
      ...
      if (fcmServerKey && fcmProjectId) {
        ...
        throw new Error(
          'No push notification service configured. Please check FCM or APNS environment variables.',
        );
      }
      ...
      if (result.success) {
        if (attempt > 0) {
          logger.system(
            `Push notification sent successfully on retry ${attempt + 1} for token ${redactedToken}`,
          );
        }
        return result;
      }
      ...
      const shouldRetry = PushRetry.isRetryable(
        result.error,
        result.failureReason,
      );
      if (!shouldRetry) {
        logger.systemError(
          `Non-retryable push error for token ${redactedToken}`,
          result.error,
        );
        return {
          ...result,
      ...
      logger.systemError(
        `Retryable push error for token ${redactedToken}, attempt ${attempt + 1}/${maxRetries}`,
        lastError,
      );
      ...
      logger.systemError(
        `Exception during push send attempt ${attempt + 1} for token ${redactedToken}`,
        lastError,
      );
...
export async function getDeviceTokensForGuests(
  ...
  try {
    ...
    logger.system(
      `Would fetch device tokens for ${guestIds.length} guests in event ${eventId}`,
    );
...
export async function sendBulkScheduledPush(
  ...
  results: Array<{
    guestId: string;
    tokenResults: Array<{ token: string; result: PushResult }>;
  }>;
}> {
  logger.system(
    `Sending scheduled push notifications to ${deliveries.length} recipients`,
  );
  ...
  const failed = processedResults.filter(
    (r) => !r.tokenResults.some((tr) => tr.result.success),
  ).length;

  logger.system(
    `Bulk scheduled push complete: ${successful} delivered, ${failed} failed`,
  );
 ...
 */
function mapFCMErrorToFailureReason(
  error: string,
