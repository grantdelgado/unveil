...
class LightweightMonitor {
  ...
  startTimer(name: string, type: PerformanceMetric['type'] = 'user-interaction'): () => void {
  ...
  logSummary() {
    ...
    Object.entries(byType).forEach(([type, durations]) => {
      ...
      console.log(`${type}: avg ${avg.toFixed(1)}ms, max ${max.toFixed(1)}ms (${durations.length} samples)`);
    });
    console.groupEnd();
  }
}
