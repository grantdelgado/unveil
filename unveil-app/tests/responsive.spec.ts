...
async function checkCriticalElements(page: Page, route: string): Promise<void> {
  switch (route) {
    case '/login':
      ...
      await expect(page.getByRole('button', { name: /continue|submit|send/i })).toBeVisible();
      break;
  ...
  test.describe(`Responsive Design - ${viewport.name} (${viewport.width}x${viewport.height})`, () => {
    ...
    for (const route of coreRoutes) {
      test(`${route.name} - Layout & Overflow Check`, async ({ page }) => {
        ...
        expect(hasOverflow).toBeFalsy(`Page has horizontal overflow on ${viewport.name}`);
        
        // Check critical elements are visible
        await checkCriticalElements(page, route.path);
        
        // Take screenshot for visual regression
        await expect(page).toHaveScreenshot(`${route.name.toLowerCase().replace(/\s+/g, '-')}-${viewport.name.toLowerCase().replace(/\s+/g, '-')}.png`);
      });
    ...
    test('Safe Area & Viewport Height Handling', async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`safe-area-${viewport.name.toLowerCase().replace(/\s+/g, '-')}.png`);
    });
  });
}
...
test.describe('Dark Mode & Accessibility', () => {
  ...
  test('Large Text Support (150% font scale)', async ({ page }) => {
    ...
    const hasOverflow = await checkForOverflow(page);
    expect(hasOverflow).toBeFalsy('Page has horizontal overflow with large text');
...
test.describe('Keyboard Navigation', () => {
  ...
  test('Virtual Keyboard Handling', async ({ page, isMobile }) => {
    ...
    const submitButton = page.getByRole('button', { name: /continue|submit|send/i });
