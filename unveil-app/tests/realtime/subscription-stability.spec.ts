...
test.describe('Guest Dashboard Realtime Stability', () => {
  ...
  test('should maintain stable subscriptions during idle period', async ({
    page,
  }) => {
    ...
    if (await debugPanel.isVisible()) {
      const healthScore = await debugPanel
  ...
  test('should recover from network offline/online cycle', async ({ page }) => {
    ...
    const reconnectMessages = consoleMessages.filter(
      (msg) => msg.includes('Back online') || msg.includes('Reconnecting'),
    );
    expect(reconnectMessages.length).toBeGreaterThan(0);

    // Should not have stuck channel errors
    const channelErrors = consoleMessages.filter(
      (msg) => msg.includes('CHANNEL_ERROR') && !msg.includes('reconnect'),
    );
  ...
  test('should handle page visibility changes gracefully', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      if (
        msg.text().includes('backgrounded') ||
        msg.text().includes('foregrounded')
      ) {
        consoleMessages.push(msg.text());
      }
    });
    ...
    const visibilityMessages = consoleMessages.filter(
      (msg) => msg.includes('backgrounded') || msg.includes('foregrounded'),
    );
  ...
  test('should not create duplicate subscriptions', async ({ page }) => {
    ...
    if (await debugPanel.isVisible()) {
      ...
      if (await subscriptionList.isVisible()) {
        const subscriptions = await subscriptionList
          .locator('.subscription-item')
          .all();
    ...
    page.on('console', (msg) => {
      if (
        msg.text().includes('already exists') ||
  ...
  test('should handle token refresh without breaking subscriptions', async ({
    page,
  }) => {
    await page.goto('/guest/events/test-event-id/home');
    await page.waitForLoadState('networkidle');
    ...
    page.on('console', (msg) => {
      if (
        msg.text().includes('Token refreshed') ||
        msg.text().includes('realtime auth')
      ) {
        consoleMessages.push(msg.text());
      }
    });
    ...
    const tokenRefreshMessages = consoleMessages.filter(
      (msg) =>
        msg.includes('Token refreshed') ||
        msg.includes('updating realtime auth'),
    );
    ...
    const errorMessages = consoleMessages.filter(
      (msg) => msg.includes('subscription error') && !msg.includes('recovered'),
    );
...
test.describe('Guest Dashboard Smoke Tests', () => {
  test('should load guest dashboard without subscription errors', async ({
    page,
  }) => {
    const consoleErrors: string[] = [];
    page.on('console', (msg) => {
    ...
    await expect(
      page.locator('[data-testid="event-messages-card"]'),
    ).toBeVisible();

    // Should not have critical subscription errors
    const subscriptionErrors = consoleErrors.filter(
      (err) => err.includes('subscription') && !err.includes('retry'),
    );
