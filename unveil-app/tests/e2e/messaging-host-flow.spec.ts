...
test.describe('Host Messaging Flow', () => {
  ...
  test.describe('Compose Message Flow', () => {
    test('host can compose and send immediate message to all guests', async ({
      page,
    }) => {
      ...
      const messageContent =
        'Welcome to our wedding celebration! We are so excited to share this special day with you.';
      ...
      await page.goto(`/host/events/${context.eventId}/messages`);
      await expect(
        page.getByText(messageContent.substring(0, 30)),
      ).toBeVisible();
    ...
    test('host can compose message with tag-based targeting', async ({
      page,
    }) => {
      await page.goto(`/host/events/${context.eventId}/messages`);
      ...
      // Fill message content
      const vipMessage =
        'Special VIP reception details will be shared closer to the date.';
    ...
    test('host can preview message recipients before sending', async ({
      page,
    }) => {
      await page.goto(`/host/events/${context.eventId}/messages`);
  ...
  test.describe('Schedule Message Flow', () => {
    test('host can schedule message for future delivery', async ({ page }) => {
      ...
      const scheduledContent =
        "Reminder: Our wedding is tomorrow! We can't wait to celebrate with you.";
      ...
      // Verify success
      await expect(
        page.getByText(/message scheduled successfully/i),
      ...
      await expect(
        page.getByText(scheduledContent.substring(0, 30)),
      ).toBeVisible();
    ...
    test('host can edit scheduled message before send time', async ({
      page,
    }) => {
      ...
      const updatedContent =
        "Updated: Our wedding is tomorrow! Don't forget to bring your dancing shoes!";
      ...
      await expect(
        page.getByText(/message updated successfully/i),
      ).toBeVisible();
    ...
    test('host can cancel scheduled message', async ({ page }) => {
      ...
      await page
        .getByRole('button', { name: /new scheduled message/i })
        .click();
      ...
      await page
        .getByLabel(/schedule time/i)
        .fill(`${futureTime.getHours().toString().padStart(2, '0')}:00`);
      ...
      await expect(
        page.getByText(cancelContent.substring(0, 20)),
      ).not.toBeVisible();
    });
  });

  test.describe('Message Delivery Tracking', () => {
    test('host can view delivery status for sent messages', async ({
      page,
    }) => {
      ...
      await expect(page.locator('[data-testid="delivery-status"]')).toHaveCount(
        5,
      ); // All 5 guests
    ...
    test('host can track read receipts for messages', async ({ page }) => {
      ...
      await expect(page.getByTestId('read-indicator-alice')).toHaveClass(
        /read/,
      );
      await expect(page.getByTestId('read-indicator-bob')).toHaveClass(/read/);
      await expect(page.getByTestId('read-indicator-carol')).toHaveClass(
        /unread/,
      );
    });
    ...
    test('host can retry failed message deliveries', async ({ page }) => {
      ...
      const failedMessage = await createMessageWithFailedDelivery(
        context.eventId,
        context.guestIds[0],
      );
  ...
  test.describe('Real-time Updates', () => {
    test('host sees real-time message responses from guests', async ({
      page,
      ...
      await expect(page.getByText(/thank you.*excited/i)).toBeVisible({
        timeout: 10000,
      });
    ...
    test('host receives real-time notifications for new responses', async ({
      page,
    }) => {
      await page.goto(`/host/events/${context.eventId}/messages`);
      ...
      await expect(page.getByTestId('message-notification')).toBeVisible({
        timeout: 5000,
      });
  ...
  test.describe('Analytics Integration', () => {
    ...
    test('analytics show accurate delivery and engagement metrics', async ({
      page,
    }) => {
      await page.goto(`/host/events/${context.eventId}/messages/analytics`);
  ...
  test.describe('Error Handling', () => {
    test('host sees appropriate error messages for failed actions', async ({
      page,
    }) => {
      await page.goto(`/host/events/${context.eventId}/messages`);
      ...
      await expect(
        page.getByText(/message content is required/i),
      ).toBeVisible();
...
async function setupHostTestData(): Promise<TestHostContext> {
  ...
  const taggedGuestIds =
    guests?.filter((g) => g.guest_tags?.includes('vip')).map((g) => g.id) || [];
...
async function cleanupHostTestData(context: TestHostContext) {
  ...
  await supabase
    ...
    .delete()
    .match({ guest_id: context.guestIds });
...
async function simulateGuestResponse(
  page: any,
  eventId: string,
  guestId: string,
...
async function createMessageWithFailedDelivery(
  eventId: string,
  guestId: string,
) {
