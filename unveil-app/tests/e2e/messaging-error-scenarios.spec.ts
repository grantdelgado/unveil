...
test.describe('Messaging Error Scenarios', () => {
  ...
  test.describe('SMS Delivery Failures', () => {
    test('handles SMS delivery failure with proper error tracking', async ({
      page,
    }) => {
      ...
      const failureTestMessage =
        'This message should fail SMS delivery due to invalid phone';
    ...
    test('SMS fallback works when push notifications fail', async ({
      page,
    }) => {
      ...
      const pushDelivery = deliveries?.find(
        (d) => d.delivery_method === 'push',
      );
    ...
    test('retry mechanism works for failed SMS deliveries', async ({
      page,
    }) => {
      await authenticateAsHost(page, context.hostUserId);
      await page.goto(`/host/events/${context.eventId}/messages`);
      ...
      const retryMessage = await createMessageWithFailedSMS(
        context.eventId,
        context.validGuestId,
      );
    ...
    test('SMS rate limiting is handled gracefully', async ({ page }) => {
      ...
      await expect(page.getByText(/rate limit.*please wait/i)).toBeVisible({
        timeout: 5000,
      });
  ...
  test.describe('Network Interruption Handling', () => {
    test('handles network failure during message composition', async ({
      page,
    }) => {
      await authenticateAsHost(page, context.hostUserId);
      await page.goto(`/host/events/${context.eventId}/messages`);
      ...
      await page
        .getByLabel(/message content/i)
        .fill('Network interruption test message');
    ...
    test('real-time connection recovers from network interruption', async ({
      page,
      ...
      await expect(guestPage.getByText(beforeInterruptionMessage)).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(guestPage.getByText(duringInterruptionMessage)).toBeVisible({
        timeout: 15000,
      });
  ...
  test.describe('Invalid Data Handling', () => {
    test('handles invalid guest data gracefully', async ({ page }) => {
      ...
      const invalidMessage = await createMessageForInvalidGuest(
        context.eventId,
      );
      ...
      // Verify error handling
      await expect(
        page.getByText(/delivery failed.*invalid guest/i),
      ).toBeVisible();
    ...
    test('validates message content before sending', async ({ page }) => {
      ...
      await expect(
        page.getByText(/message.*too long|character limit/i),
      ).toBeVisible();
    ...
    test('handles unauthorized access attempts', async ({ page }) => {
      ...
      await expect(
        page.getByText(/not found|access denied|unauthorized/i),
      ).toBeVisible();
    ...
    test('validates tag targeting with non-existent tags', async ({ page }) => {
      ...
      await expect(
        page.getByText(/no recipients.*found|0 recipients/i),
      ).toBeVisible();
  ...
  test.describe('CRON Processor Error Recovery', () => {
    test('CRON processor handles database connection errors', async ({
      page,
    }) => {
    ...
    test('CRON processor handles partial batch failures', async ({ page }) => {
      ...
      const validProcessed = processedMessages?.find(
        (m) => m.id === validMessage.id,
      );
      const invalidProcessed = processedMessages?.find(
        (m) => m.id === invalidMessage.id,
      );
    ...
    test('CRON processor timeout handling', async ({ page }) => {
      ...
      await supabase.from('scheduled_messages').insert(messageInserts);
  ...
  test.describe('Error Recovery & User Experience', () => {
    ...
    test('graceful degradation when features are unavailable', async ({
      ...
      await expect(
        page.getByText(/analytics.*unavailable|unable to load analytics/i),
      ).toBeVisible({ timeout: 10000 });
      ...
      await page
        .getByLabel(/message content/i)
        .fill('Graceful degradation test');
      await page.getByRole('button', { name: /send now/i }).click();

      await expect(page.getByText(/message sent successfully/i)).toBeVisible();
    });

    test('user-friendly error messages for common scenarios', async ({
      page,
    }) => {
      await authenticateAsHost(page, context.hostUserId);
      await page.goto(`/host/events/${context.eventId}/messages`);
...
async function cleanupErrorTestData(context: ErrorTestContext) {
  await supabase
    .from('message_deliveries')
    .delete()
    .in('guest_id', [context.validGuestId, context.invalidGuestId]);
