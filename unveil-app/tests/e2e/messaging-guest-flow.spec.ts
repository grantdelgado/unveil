...
test.describe('Guest Messaging Flow', () => {
  ...
  test.describe('Message Reception & Display', () => {
    test('guest can view broadcast messages sent to all guests', async ({
      page,
    }) => {
      // Create broadcast message from host
      const broadcastContent =
        'Welcome everyone! We are thrilled you can join our special day.';
      const messageId = await createHostMessage(
        context.eventId,
        broadcastContent,
        'broadcast',
      );
    ...
    test('guest only sees messages targeted to them via tags', async ({
      page,
    }) => {
      // Create VIP-only message
      const vipContent =
        'Special VIP reception information for our closest family and friends.';
      const vipMessageId = await createHostMessage(
        context.eventId,
        vipContent,
        'tags',
        ['vip'],
      ...
      const familyContent =
        'Family photo session details - please arrive 30 minutes early.';
      await createHostMessage(context.eventId, familyContent, 'tags', [
        'family',
      ]);
    ...
    test('guest cannot see messages from other events', async ({ page }) => {
      ...
      const otherEventContent =
        'Message from other event - should not be visible';
    ...
    test('guest sees real-time message updates', async ({
      page,
      ...
      const realtimeContent =
        "Just posted: Updated timeline for tomorrow's events!";
      ...
      await expect(page.getByText(realtimeContent)).toBeVisible({
        timeout: 15000,
      });
  ...
  test.describe('Message Reading & Tracking', () => {
    test('guest can mark messages as read and tracking updates', async ({
      page,
    }) => {
      ...
      await expect(page.getByTestId('message-read-indicator')).toHaveClass(
        /read/,
      );
    ...
    test('guest sees unread message indicators', async ({ page }) => {
      ...
      const unreadMessageId = await createHostMessage(
        context.eventId,
        unreadContent,
        'broadcast',
      );
  ...
  test.describe('Message Responses', () => {
    test('guest can respond to host messages', async ({ page }) => {
      ...
      const responseContent =
        'Thank you so much! We are incredibly excited to celebrate with you both!';
    ...
    test('guest can send multiple responses in a conversation', async ({
      page,
    }) => {
      await authenticateAsGuest(page, context.guest1Phone);
      await page.goto(`/guest/events/${context.eventId}/home`);
      ...
      await page.getByRole('button', { name: /reply/i }).click();
      await page
        .getByLabel(/your response/i)
        .fill('First response in conversation');
      await page.getByRole('button', { name: /send reply/i }).click();
      ...
      await page
        .getByLabel(/your response/i)
        .fill('Second response with additional thoughts');
      await page.getByRole('button', { name: /send reply/i }).click();
      ...
      await expect(
        page.getByText('First response in conversation'),
      ).toBeVisible();
      await expect(
        page.getByText('Second response with additional thoughts'),
      ).toBeVisible();
    });

    test('guest response triggers real-time update for host', async ({
      page,
      context: browserContext,
    }) => {
      ...
      await expect(hostPage.getByText(liveResponseContent)).toBeVisible({
        timeout: 15000,
      });
      await expect(
        hostPage.getByTestId('new-response-notification'),
  ...
  test.describe('Guest Isolation & Security', () => {
    test('guest cannot access other guests responses', async ({ page }) => {
      ...
      const guest2Response =
        'Private response from guest 2 - should not be visible to guest 1';
      await createGuestResponse(
        context.eventId,
        context.guest2Id,
        guest2Response,
      );
    ...
    test('guest cannot access host compose or analytics pages', async ({
      page,
    }) => {
      await authenticateAsGuest(page, context.guest1Phone);
      ...
      await expect(
        page.getByText(/access denied|not authorized|404/i),
      ).toBeVisible();
    ...
    test('guest sees only their own RSVP and profile information', async ({
      page,
    }) => {
      await authenticateAsGuest(page, context.guest1Phone);
      await page.goto(`/guest/events/${context.eventId}/home`);
    ...
    test('guest cannot manipulate URL parameters to access other data', async ({
      page,
    }) => {
      await authenticateAsGuest(page, context.guest1Phone);
      ...
      await expect(
        page.getByText(/event not found|access denied/i),
      ).toBeVisible();

      // Try to access with different guest parameters
      await page.goto(
        `/guest/events/${context.eventId}/home?guestId=${context.guest2Id}`,
      );
  ...
  test.describe('Mobile & Responsive Experience', () => {
    ...
    test('guest can use pull-to-refresh for message updates', async ({
      page,
    }) => {
...
async function createHostMessage(
  eventId: string,
  content: string,
  type: 'broadcast' | 'tags' | 'direct',
  tags?: string[],
): Promise<string> {
