...
test.describe('RSVP-Lite Functionality', () => {
  ...
  test('guest can decline event with reason', async ({ page }) => {
    ...
    const cantMakeItButton = page.locator('button', {
      hasText: "Can't make it?",
    });
    ...
    await expect(declineBanner).toContainText(
  ...
  test('host can see declined guests and clear decline status', async ({
    page,
  }) => {
    ...
    const declinedGuest = page.locator(
      '[data-testid="guest-declined-indicator"]',
    );
  ...
  test('messaging excludes declined guests by default', async ({ page }) => {
    ...
    const recipientSelect = page.locator(
      'select[data-testid="recipient-filter"]',
    );
    ...
    expect(parseInt(allGuestsCount || '0')).toBeGreaterThan(
