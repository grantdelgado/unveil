...
test.describe('Messaging Performance & Load Tests', () => {
  ...
  test.describe('Large Scale Message Delivery', () => {
    test('can deliver message to 100+ guests within acceptable time', async ({
      page,
    }) => {
      ...
      const bulkMessageContent =
        'This is a bulk message test for 100+ guests to validate delivery performance.';
      ...
      console.log(
        `✅ Bulk message delivery time: ${(deliveryTime / 1000).toFixed(2)}s for ${context.largeGuestIds.length} guests`,
      );
      ...
      expect(successfulDeliveries.length).toBeGreaterThan(
        context.largeGuestIds.length * 0.9,
      ); // 90+ success rate
    });

    test('can handle concurrent message sending from multiple tabs', async ({
      page,
        ...
        const promise = async () => {
          ...
          try {
            await messagePage
              ...
              .fill(`Concurrent message test ${i + 1}`);
            await messagePage
              .getByRole('button', { name: /send now/i })
              .click();
            await expect(
              messagePage.getByText(/message sent successfully/i),
      ...
      const successfulConcurrent =
        context.performanceMetrics.concurrentUserMetrics.filter(
          (m) => m.success,
        );
    ...
    test('message delivery maintains performance with large guest lists', async ({
      page,
    }) => {
      ...
      for (const guestCount of testSizes) {
        ...
        const messageId = await createDirectMessage(
          context.eventId,
          `Performance test for ${guestCount} guests`,
          targetGuestIds,
        );
        ...
        console.log(
          `📊 Delivery time for ${guestCount} guests: ${(deliveryTime / 1000).toFixed(2)}s`,
        );
      }
  ...
  test.describe('Real-time Performance Under Load', () => {
    test('supports 50+ concurrent real-time listeners', async ({
      page,
      context: browserContext,
    }) => {
      ...
      const propagationPromises = realtimePages.map(
        async (guestPage, index) => {
          const startTime = performance.now();
          await expect(guestPage.getByText(realtimeTestMessage)).toBeVisible({
            timeout: 15000,
          });
          const endTime = performance.now();
      ...
      const maxLatency = Math.max(
        ...context.performanceMetrics.realtimeLatency,
      );
      const avgLatency =
        context.performanceMetrics.realtimeLatency.reduce((a, b) => a + b, 0) /
        context.performanceMetrics.realtimeLatency.length;

      console.log(
        `📡 Real-time propagation - Avg: ${avgLatency.toFixed(0)}ms, Max: ${maxLatency.toFixed(0)}ms`,
      );
    ...
    test('real-time subscriptions handle rapid message bursts', async ({
      page,
      context: browserContext,
    }) => {
      ...
      console.log(
        `⚡ Burst test: ${burstCount} messages delivered successfully`,
      );

      await guestPage.close();
    });

    test('memory usage remains stable during extended real-time sessions', async ({
      page,
    }) => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        await createDirectMessage(
          context.eventId,
          `Extended session test ${i}`,
        );

        // Wait for real-time update
        await expect(page.getByText(`Extended session test ${i}`)).toBeVisible({
          timeout: 10000,
        });
      ...
      if (initialMemory && finalMemory) {
        ...
        console.log(
          `🧠 Memory growth during session: ${memoryGrowthMB.toFixed(2)}MB`,
        );
  ...
  test.describe('Scheduled Message Processing Performance', () => {
    test('CRON processor handles 20+ scheduled messages efficiently', async ({
      page,
    }) => {
      ...
      console.log(
        `⏰ CRON processed ${scheduledCount} messages in ${(processingTime / 1000).toFixed(2)}s`,
      );
      ...
      const successfullyProcessed =
        processedMessages?.filter((m) => m.status === 'sent') || [];
      expect(successfullyProcessed.length).toBeGreaterThan(
        scheduledCount * 0.9,
      );
    });

    test('CRON processor handles errors gracefully without blocking', async ({
      page,
    }) => {
      ...
      console.log(
        `🔧 Error handling test: ${result.successful} successful, ${result.failed} failed`,
      );
    });
  });

  test.describe('Query Performance & Database Load', () => {
    test('message queries perform well with large datasets', async ({
      page,
    }) => {
      ...
      const queryTests = [
        ...
        },
        {
          ...
          query: () =>
            supabase
              .from('message_deliveries')
              .select('*, messages(*), event_guests(*)')
              .eq('messages.event_id', context.eventId),
        },
        {
      ...
      for (const queryTest of queryTests) {
        ...
        console.log(
          `🗃️ ${queryTest.name}: ${avgQueryTime.toFixed(0)}ms average`,
        );
    ...
    test('page rendering performance with large message threads', async ({
      page,
    }) => {
      // Create a message with many responses to test rendering performance
      const parentMessageId = await createDirectMessage(
        context.eventId,
        'Parent message for large thread test',
      );
      ...
      await expect(
        page.getByText(`Response ${responseCount} to test rendering`),
      ).toBeVisible({ timeout: 30000 });
      ...
      console.log(
        `🎨 Large thread render time: ${(renderTime / 1000).toFixed(2)}s for ${responseCount} messages`,
      );
      ...
      const visibleResponses = await page
        .locator('[data-testid="message-response"]')
        .count();
      expect(visibleResponses).toBe(responseCount);
    });

    test('analytics queries maintain performance with large datasets', async ({
      page,
    }) => {
      ...
      console.log(
        `📊 Analytics load time: ${(analyticsLoadTime / 1000).toFixed(2)}s`,
      );
  ...
  test.describe('Performance Reporting', () => {
    test('generate performance summary report', async ({ page }) => {
      ...
      const avgDeliveryTime =
        metrics.messageDeliveryTime.reduce((a, b) => a + b, 0) /
        metrics.messageDeliveryTime.length;
      const avgQueryTime =
        metrics.queryResponseTime.reduce((a, b) => a + b, 0) /
        metrics.queryResponseTime.length;
      const avgRenderTime =
        metrics.renderTime.reduce((a, b) => a + b, 0) /
        metrics.renderTime.length;
      const avgRealtimeLatency =
        metrics.realtimeLatency.reduce((a, b) => a + b, 0) /
        metrics.realtimeLatency.length;
...
async function setupLoadTestData(): Promise<LoadTestContext> {
  ...
  for (let i = 0; i < largeGuestCount; i++) {
    guestInserts.push({
      ...
      guest_tags:
        i % 4 === 0 ? ['vip'] : i % 3 === 0 ? ['family'] : ['friends'],
      user_id: null,
    });
  }
...
async function cleanupLoadTestData(context: LoadTestContext) {
  ...
  await supabase
    .from('message_deliveries')
    .delete()
    .in('guest_id', context.largeGuestIds);
